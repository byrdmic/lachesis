...
export function generateLog(project: ProjectDefinition): string {
  ...
  const whatHappened = project.sessionLog.length > 0
    ? project.sessionLog
        .slice(0, 5)
        .map((entry) => `- **${entry.phase}:** ${entry.question.slice(0, 50)}...`)
        .join("\n")
    : "- Initial project creation via Lachesis";

  const decisions = [
    project.vision.oneLinePitch ? `- Defined project pitch: "${project.vision.oneLinePitch}"` : null,
    project.vision.primaryAudience ? `- Identified primary audience: ${project.vision.primaryAudience}` : null,
    project.solution.approach ? `- Established approach: ${project.solution.approach.slice(0, 80)}...` : null,
  ]
    .filter(Boolean)
