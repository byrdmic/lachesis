...
import type { PlanningLevel, InterviewDepth, QuestionMode } from "../../core/project/types.ts";
...
import { buildCoachingPrompt, buildFirstQuestionPrompt } from "../../ai/prompts.ts";
...
type InterviewPhaseProps = {
  ...
  projectName: string;
  oneLiner: string;
  onComplete: (extractedData: ExtractedProjectData, conversationLog: ConversationMessage[]) => void;
  onCancel: () => void;
};
...
export function InterviewPhase({
  ...
  const generateFirstQuestion = async () => {
    ...
    const prompt = buildFirstQuestionPrompt(projectName, oneLiner, planningLevel, depth);
  ...
  const handleUserAnswer = useCallback(
    async (answer: string) => {
      ...
      const takeWheelTriggers = ["take the wheel", "write it for me", "you decide", "summarize"];
      const isWrapUp = takeWheelTriggers.some((t) => answer.toLowerCase().includes(t));
    ...
    [state.messages, state.coveredTopics, config, planningLevel, depth, projectName, oneLiner]
  );
  ...
  if (state.step === "generating_question" || state.step === "extracting_data") {
    return (
      <Box flexDirection="column" padding={1}>
        ...
        <Box marginTop={1}>
          ...
          <Text> {state.step === "extracting_data" ? "Processing..." : "Thinking..."}</Text>
        </Box>
      </Box>
    );
  }
