...
import { getConfigDir, getConfigPath, getDefaultVaultPath, getPlatformDisplayName } from "./paths.ts";
...
export function loadConfig(): ConfigLoadResult {
  ...
  try {
    ...
    const message = `Created config at ${configPath}\n` +
 ...
 */
export function saveConfig(config: LachesisConfig): { success: boolean; error?: string } {
  const configPath = getConfigPath();
  const configDir = getConfigDir();
...
): { success: boolean; config?: LachesisConfig; error?: string } {
